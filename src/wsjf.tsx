...
function GetStoredFields(): IPromise<any> {
  ...
  VSS.getService<IExtensionDataService>(VSS.ServiceIds.ExtensionData).then(
    (dataService: IExtensionDataService) => {
      dataService
        .getValue<StoredFieldReferences>("storedFields")
...
function updateWSJFOnForm(storedFields: StoredFieldReferences) {
  getWorkItemFormService().then((service) => {
    service.getFields().then((fields: TFS_Wit_Contracts.WorkItemField[]) => {
      var matchingBusinessValueFields = fields.filter(
        (field) => field.referenceName === storedFields.bvField
      );
      var matchingTimeCriticalityFields = fields.filter(
        (field) => field.referenceName === storedFields.tcField
      );
      var matchingRROEValueFields = fields.filter(
        (field) => field.referenceName === storedFields.rvField
      );
      var matchingEffortFields = fields.filter(
        (field) => field.referenceName === storedFields.effortField
      );
      var matchingWSJFFields = fields.filter(
        (field) => field.referenceName === storedFields.wsjfField
      );
      ...
      service
        ...
        .then((values) => {
          var businessValue =
            values[storedFields.bvField] !== undefined || NaN
              ? Number(values[storedFields.bvField])
              : 0;
          var timeCriticality =
            values[storedFields.tcField] !== undefined || NaN
              ? Number(values[storedFields.tcField])
              : 0;
          var rroevalue =
            values[storedFields.rvField] !== undefined || NaN
              ? Number(values[storedFields.rvField])
              : 0;
          var effort =
            values[storedFields.effortField] !== undefined || NaN
              ? Number(values[storedFields.effortField])
              : 0;
          ...
          if (effort > 0) {
            ...
            if (roundTo > -1) {
              wsjf =
                Math.round(wsjf * Math.pow(10, roundTo)) /
                Math.pow(10, roundTo);
            }
          }
...
function updateWSJFOnGrid(
  ...
  client
    .getWorkItem(workItemId, wsjfFields)
    .then((workItem: TFS_Wit_Contracts.WorkItem) => {
      if (
        storedFields.wsjfField !== undefined &&
        storedFields.rvField !== undefined
      ) {
        var businessValue = Number(workItem.fields[storedFields.bvField]) || 0;

        var timeCriticality =
          Number(workItem.fields[storedFields.tcField]) || 0;
        ...
        if (effort > 0) {
          ...
          if (roundTo > -1) {
            wsjf =
              Math.round(wsjf * Math.pow(10, roundTo)) / Math.pow(10, roundTo);
          }
        }
      ...
      GetStoredFields().then(
        (storedFields: StoredFieldReferences) => {
          if (
            ...
            if (
              args.changedFields[storedFields.bvField] !== undefined ||
...
VSS.register(
  `${extensionContext.publisherId}.${extensionContext.extensionId}.wsjf-work-item-form-observer`,
  formObserver
);
VSS.register(
  `${extensionContext.publisherId}.${extensionContext.extensionId}.wsjf-contextMenu`,
  contextProvider
);
